...
public class Calculator{
    public int calculator(String input) {//ロジック層　動詞ぽい名前に設定しなおし
        // 文字列を整理 input deln の意味が弱い修正
        String input_deln =input.replaceAll("\\s+","");//Stringクラスの関数を使って空白(\\n)削除

        //文字列を分解　static finalで命名する
        final Pattern pattern = Pattern.compile("(\\d+)([-+*/])(\\d+)");//数字列(\\d+),数字以外列(\\D+),数字列(\\d+)のパターンを作成
        Matcher match = pattern.matcher(input_deln);//このパターンでグループ化
        ...
        String operator = match.group(2);//演算子,記号を代入
